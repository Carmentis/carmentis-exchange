...
export class AuthService {
    ...
    async generateChallenge(): Promise<{
        ...
        this.logger.verbose(
            `Generated challenge: ${challengeEntity.id} -> ${challengeEntity.challenge}`,
        );
    ...
    async verifyChallenge(
        ...
        try {
            ...
            const decodedPublicKey =
                this.signatureEncoder.decodePublicKey(publicKey);
            const hexEncoder = EncoderFactory.bytesToHexEncoder();

            // Verify the signature
            this.logger.verbose(
                `Verifying challenge signature: ${challenge.challenge} -> ${signature}`,
            );
            ...
            if (!isValid) {
                this.logger.error(
                    `Invalid signature for challenge: ${challengeId}`,
                );
            ...
            if (!this.authorizedKeysService.isAuthorized(publicKey)) {
                ...
                throw new UnauthorizedException(
                    'Unauthorized key: Your public key is not in the authorized keys list',
                );
            }
